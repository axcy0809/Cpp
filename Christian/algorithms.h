...
auto multiply_with_value(C &ctnr, typename C::value_type value) {
  ...
  std::for_each(ctnr.begin(), ctnr.end(), func);
  //std::transform(ctnr.begin(), ctnr.end(), ctnr.begin(), 
  //[value](typename C::value_type& item){return item * value;});
};

// reverse order of elements
template <typename C> 
auto sort_descending(C &ctnr) {
